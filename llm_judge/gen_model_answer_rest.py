...
def rest_forward(input_ids, model, tokenizer, max_new_token, temperature, top_p, datastore, num_draft, token_spans, max_steps=1024):
    ...
    for idx in tqdm(range(max_steps), desc="gen_model_answer_rest.rest_forward.0"): 
...
def run_eval(
    ...
    ans_handles = []
    for i in tqdm(range(0, len(questions), chunk_size), desc="gen_model_answer_rest.run_eval.0"):
        ans_handles.append(
            get_answers_func(
                model_path,
                model_id,
...
def get_model_answers(
    ...
    accept_lengths_tree = []
    for question in tqdm(questions[:2], desc="gen_model_answer_rest.get_model_answers.0"):
